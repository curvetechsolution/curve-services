...
// ── Web Features ──────────────────────────────────────────────────
// NOTE: Everything below is a pure add-on now — nothing is force-included.
// The "Build Your Own Plan" total therefore always starts at Rs. 0 and the
// user explicitly turns on (or leaves off) every single item, including
// WhatsApp Button, Mobile Responsive and the number of pages.
const WEB_FEATURES = {
  service: [
    { key:"pages",      icon:"📄", label:"Number of Pages",        type:"counter", min:0, max:20, default:0, basePrice:800,  unit:"page",  desc:"Price per page" },
    { key:"whatsapp",   icon:"💬", label:"WhatsApp Button",         type:"toggle",  price:0,     included:false, desc:"CTA button linking to WhatsApp" },
    ...
    { key:"mobile",     icon:"📱", label:"Mobile Responsive",        type:"toggle",  price:0,     included:false, desc:"Works on all screen sizes" },
  ],
  ecom: [
    { key:"pages",      icon:"📄", label:"Number of Pages",          type:"counter", min:0, max:20, default:0, basePrice:800,  unit:"page",  desc:"Price per page" },
...
// Everything is now selected à la carte, so there is no hidden starting
// price — the base price for both plan types is Rs. 0.
  ...
  return (
    <div style={{ marginBottom:40, background:"#fff", borderRadius:24, padding:"20px 18px 100px", boxShadow:"0 4px 32px rgba(0,0,0,.07)", border:"1px solid #f0f4f8" }}>
      <div style={{ textAlign:"center", marginBottom:16 }}>
        ...
        <p style={{ color:"#94a3b8", fontSize:13, margin:0 }}>Starts at Rs. 0 — select every feature you want, including pages, WhatsApp button & Domain/Hosting · Price updates live</p>
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
        <div>
          ...
          <div className="web-feat-grid">
            {features.map(f => (
              <div key={f.key} style={{ background: (f.type==="toggle"?sel[f.key]:sel[f.key]>0) ? `${color}08` : "#f8fafc", border:`1.5px solid ${(f.type==="toggle"?sel[f.key]:sel[f.key]>0)?color+"30":"#e8edf2"}`, borderRadius:10, padding:"8px 10px", transition:"all .2s", height:"100%", boxSizing:"border-box" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, height:"100%" }}>
                  ...
                  {f.type === "toggle" && (
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ fontSize:12, fontWeight:700, color: f.price ? color : "#10b981" }}>{f.price ? fmtPrice(f.price) : "Free"}</span>
        ...
        <div style={{ background:`linear-gradient(135deg,${color}10,${color}04)`, border:`2px solid ${color}30`, borderRadius:18, padding:"24px 28px" }}>
          ...
          <div style={{ fontSize:11, color:"#ef4444", fontWeight:600, marginBottom:16 }}>❌ Nothing is included by default — toggle on every feature you want above (WhatsApp Button, Domain & Hosting, Pages, etc.)</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:"6px 20px", marginBottom:18, minHeight:36 }}>
            {picked.length === 0 && <div style={{ color:"#94a3b8", fontSize:13 }}>No features selected yet — total starts at Rs. 0</div>}
            {picked.map(f => (
              <div key={f.key} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:12, color:"#374151", gap:8 }}>
                ...
                <span style={{ fontWeight:700, color, flexShrink:0, fontSize:11 }}>
                  {f.type==="counter" ? fmtPrice((sel[f.key]-f.min)*f.basePrice) : (f.price ? fmtPrice(f.price) : "Free")}
                </span>
              </div>
            ))}
          </div>
...
function SMMService({ color }) {
  const [mode, setMode] = useState("packages");
  // Custom builder ab bilkul khaali/zero se shuru hota hai — koi platform ya
  // post pehle se select nahi, user khud sab kuch chunta hai.
      ...
      {mode==="custom" && (
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ background:`linear-gradient(135deg,${color}10,${color}04)`, border:`2px solid ${color}30`, borderRadius:18, padding:"24px 28px" }}>
            ...
            <div style={{ borderTop:`1.5px dashed ${color}40`, paddingTop:14, marginBottom:16, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontSize:12, color:"#64748b" }}>Selected Platforms</div>
                <div style={{ fontSize:13, fontWeight:700, color:"#0f172a" }}>{plats.length ? plats.map(id=>PLATS_DEF.find(p=>p.id===id)?.label).join(", ") : "None selected yet"}</div>
              </div>
              <div style={{ textAlign:"right" }}>
